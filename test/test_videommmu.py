...
from dataloader.videommmu import VideoMMMU_Bench, videoperception_aggregate_results, process_out_list, videoperception_process_results
import argparse
import multiprocessing
from utils import deep_copy_complex_list, select_output, extract_segments, resample_frames
...
def build_model(model_path, max_frames, num_reflection):
    ...
    from model_transformers_internvl3 import InternVL_Transformers
    from model_transformers import QwenVL_Transformers

    if "InternVL3" in model_path:
        model = InternVL_Transformers(model_path, process_out_list, max_thinking_tokens=4096, max_frames=max_frames, num_reflection=num_reflection)
    else:
    ...
    try:
        ...
        def process_batch(batch_video_paths, batch_text_input, batch_image, batch_doc):
            ...
            ### BoN
                ...
                if "<OPEN-QUESTION>" in inputs[0]['text']:
                    output = model_bon(batch_video_paths, batch_text_input, query_image=batch_image, doc=batch_doc, inputs=input1)
                else:
                    #### The first round ####
                    ...
                    if type(output)==tuple:
                        ...
                        # Temporal zoom in (Controller)
                        if model_bon.temporal_zoom_in:
                            print("Temporal zoom in.")
                            filtered_segs = extract_segments(key_frames[0], model_bon.fps_list[0])
                            video_new = resample_frames(batch_video_paths[0], inputs, filtered_segs, model_bon.fps_list[0], target_total_frames=args.max_frames, bias_ratio=0.5)
                            inputs[0]['video'] = video_new
                            # model_bon.fps_list[0] = None

                            if model_bon.spatial_zoom_in:
                                # Spatial zoom in (Controller)
                                print("Spatial zoom in.")
                                output_keyframe = model_bon(batch_video_paths, batch_text_input, query_image=batch_image, doc=batch_doc, key_frames=key_frames, inputs=inputs, tem=0.01, top_p=0.5, top_k=5)
                            else:
                                output_keyframe = model_bon(batch_video_paths, batch_text_input, query_image=batch_image, doc=batch_doc, inputs=inputs, tem=0.01, top_p=0.5, top_k=5)
                        ...
                        if len(pred_list) == 2:            # use when Bo2, tem=0.0
                            ...
                            if pred not in pred_list or pred == "No Answer Found":
                                output = output[2]   # response with max score
                            else:
                                output = output_keyframe

            print(batch_doc[0]["id"], "GT:", batch_doc[0]["answer"], "Pred:", output[0]["text"])
            for out in output:
        ...
        metrics = [videoperception_process_results(docs[i], results[i]) for i in range(len(docs))]
...
def evaluate(args, num_gpus, gpu_list):
    ...
    videommmu = VideoMMMU_Bench(args.data_dir, add_asr=not args.disable_asr, asr_dir=args.asr_dir)
...
def main():
    ...
    metrics_path = './logs/metrics_' + args.exp_name + '.json'
    with open(metrics_path, "w", encoding="utf-8") as file:
        json.dump(metrics, file, ensure_ascii=False, indent=4)
        
    results_path = './logs/results_' + args.exp_name + '.json'
