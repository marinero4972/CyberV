...
class VideoMME_Bench:
    def __init__(self, data_dir, add_asr=True, asr_dir=None):
...
def videomme_doc_to_text(doc, lmms_eval_specific_kwargs=None):
    option_prompt = "Select the best answer to the following multiple-choice question based on the video and the subtitles(if provided). Respond with only the letter (A, B, C, or D) of the correct option."
...
def videomme_process_results_new(doc, pred):
    """
    Args:
        doc: a instance of the eval dataset
        pred: the prediction of the model
    ...
    sub_category = doc["sub_category"]
    task_category = doc["task_type"]
    data_dict = {"question_id": doc["question_id"], "duration": doc["duration"], "category": category, "sub_category": sub_category, "task_category": task_category, "pred_answer": pred_ans, "answer": doc["answer"], "response":pred}
