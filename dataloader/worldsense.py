...
class WorldSense_Bench:
    def __init__(self, data_dir=None, add_asr=True, asr_dir=None):
    ...
    def process_data(self, line):
        ...
        if osp.exists(asr_path) and self.add_asr:
            ...
            subtitles = ''
        video_path = osp.join(self.data_dir, 'videos', line['video'] + '.mp4')
        ...
        if contains_timestamp(text_input):
            text_input = text_input + "<HAS TIMESTAMP>"
        else:
            for special_string in ['start','begin','end', 'when', 'while','show','introduc','during','mention','discuss']:
                if special_string in line["question"].lower() or special_string in line["candidates"].lower():
                    text_input = text_input + "<IMPLICIT TIMESTAMP>"
                    break
